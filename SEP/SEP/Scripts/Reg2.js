        ...
        $.ajax({
            ...
            success: function (data) {
               ...
               for (var i = 0 ; i < text ; i++) {
                   ...
                   $("<div class='desc' id ='ak2'> <div class='thumb'><i><span class='photo'><img alt='avatar' src=" + data.data2[i] + " width='30px' height='30px' ></span></i></div><div class='details'> <p><muted>" + data.data3[i] + "</muted><br /> <span style='font-size:11px'><a href='#' id='sp2'>" + t[0] + "</a>" + t[1] + "</span><br /><input type='button' class='btn btn-default btn-xs' value ='Confirm' id='cn'/><input type='button' class='btn btn-default btn-xs' value ='Decline' id='cnf1'/></p></div></div><p style='display:none' id ='akakav'>" + data.data4[i] + "</p>")
                   .appendTo('.Request1');
                }
...
$(document).ready(function () {
    $(document).on('click', '#cnf1', function () {
        ...
        var p = t2;
        $("#akakav").addClass("clicked3");
        var t3 = $(".clicked3").html();
        console.log(t3 + "Ayesh"+p[0]);
        if (t == "Confirm") {
            $.ajax({
                url: '/Dropdwn/AddLec',
                data: {Name : t2},
